 ...
 * 【关键修复 2026-06-23】原地取/送货改用 utils.work()，成功即清零卡死计数防误杀。
 */
...
module.exports = {
  run(creep) {
    ...
    if (creep.memory.working) {
      ...
      if (target) {
        if (utils.work(creep, 'transfer', target, RESOURCE_ENERGY) === ERR_NOT_IN_RANGE) utils.moveTo(creep, target, '#ffffff');
      } else {
      ...
      if (containers.length) {
        ...
        if (utils.work(creep, 'withdraw', containers[0], RESOURCE_ENERGY) === ERR_NOT_IN_RANGE) utils.moveTo(creep, containers[0], '#ffaa00');
        return;
      }
      ...
      if (dropped) {
        if (utils.work(creep, 'pickup', dropped) === ERR_NOT_IN_RANGE) utils.moveTo(creep, dropped, '#ffaa00');
        return;
      }
      // 实在没货，去 source 旁等
